...
\*\*AWS (Amazon Web Services)\*\*는 **아마존에서 제공하는 클라우드 컴퓨팅 플랫폼**입니다.
전 세계 데이터센터(Region, AZ)를 기반으로 **서버, 스토리지, 데이터베이스, AI/ML, 보안, 네트워킹** 등
IT 인프라를 필요할 때마다 빌려 쓸 수 있는 **IaaS / PaaS / SaaS 서비스**를 제공합니다.
...
2. **Region & AZ (Availability Zone)**

   * **Region** → AWS가 운영하는 물리적 데이터센터 묶음 (서울: `ap-northeast-2`)
   * **AZ** → Region 내 독립된 데이터센터 (고가용성 확보)

3. **글로벌 인프라**

   * 전 세계 30개+ Region, 90개+ AZ 운영
   * 서비스 장애 시 자동으로 다른 AZ로 Failover 가능
...
1. **컴퓨팅**
   ...
   * **EC2**: 가상 서버 (필요한 만큼 인스턴스 생성)
   * **Lambda**: 서버리스(Serverless) 함수 실행
   * **ECS / EKS**: 컨테이너 오케스트레이션 (Docker, Kubernetes)

2. **스토리지**

   * **S3**: 객체 스토리지 (파일 저장, 정적 웹사이트 호스팅)
   * **EBS**: 블록 스토리지 (EC2 전용 디스크)
   * **Glacier**: 장기 백업용 저렴한 스토리지

3. **데이터베이스**

   * **RDS**: 관리형 관계형 DB (MySQL, PostgreSQL 등)
   * **DynamoDB**: 서버리스 NoSQL DB
   * **Aurora**: MySQL 호환 고성능 DB

4. **네트워킹**

   * **VPC**: 가상 네트워크 (서브넷, 라우팅, 보안그룹)
   * **CloudFront**: CDN (전 세계 엣지 로케이션에서 콘텐츠 제공)
   * **Route 53**: 도메인 관리 & 트래픽 라우팅

5. **보안 & 모니터링**

   * **IAM**: 사용자/권한 관리
   * **CloudWatch**: 로그 & 모니터링
   * **CloudTrail**: API 호출 기록 감사

---

#### 🔹 AWS 사용 장점

* **초기 비용 없음** → 필요한 만큼 사용한 만큼만 지불 (Pay-as-you-go)
* **확장성 & 유연성** → 트래픽 급증 시 자동 스케일링
* **고가용성** → 여러 AZ에 자동 분산 배포
* **보안 강화** → 글로벌 인증 (ISO, SOC, HIPAA) 지원
...
* **개인 프로젝트 배포**
  ...
  * EC2에 서버 띄우고 → RDS 연결 → S3에 이미지 저장
* **팀 해커톤/스타트업**

  * Lambda + API Gateway로 서버리스 API 구축
* **대규모 서비스**
